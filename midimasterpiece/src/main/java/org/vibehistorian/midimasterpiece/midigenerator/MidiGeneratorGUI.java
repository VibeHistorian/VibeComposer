...
public class MidiGeneratorGUI extends JFrame
	...
	// arp gen settings
	JTextField randomArpsCount;
	JCheckBox randomArpsOnCompose;
	...
	private void initChordGenSettings(int startY, int anchorSide) {
		...
		randomChordsCount = new JTextField("2", 2);
	...
	public void actionPerformed(ActionEvent ae) {
		...
		if (ae.getActionCommand() == "RandomizeInst"
			...
			if (!melodyLock.isSelected()) {
				...
				if (arpMelodyLockInst.isSelected()) {
					MidiUtils.selectJComboBoxByInst(melodyInst,
							MidiUtils.INST_POOLS.get(POOL.PLUCK), arpPanels.get(0).getInstrument());
				} else {
		...
		if (ae.getActionCommand() == "RandomizeBpmTrans" || (ae.getActionCommand() == "Compose"
			...
			if (arpAffectsBpm.isSelected()) {
				int highestArpPattern = arpPanels.stream().map(e -> e.getPatternRepeat())
		...
		
		{
			...
			randomArpsCount.setText("" + arpPanels.size());
			randomDrumsCount.setText("" + drumPanels.size());
			
			instrumentTabPane.setTitleAt(0, "Chords (" + randomChordsCount.getText() + ")");
			instrumentTabPane.setTitleAt(1, "Arps (" + randomArpsCount.getText() + ")");
			instrumentTabPane.setTitleAt(2, "Drums (" + randomDrumsCount.getText() + ")");
	...
	private void createRandomArpPanels(int panelCount) {
		...
		for (int i = 0; i < panelCount; i++) {
			...
			if (randomArpLockInst.isSelected()) {
